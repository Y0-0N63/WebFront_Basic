...
function check1(btn) {
  ...
  // 현재 버튼의 배경색을 얻어와 저장하기
  const bgColor = btn.style.backgroundColor;
  
  // 버튼이 클릭될 때마다 yellow <-> pink (toggle)
  if(bgColor == "yellow") {
...
// 고전 이벤트 모델 작성법 : 요소.이벤트리스너 = 이벤트핸들러
test1a.onclick = function() {
  alert("고전 이벤트 모델 확인 버튼이 클릭되었습니다.")
};

// 고전 이벤트 모델 제거
// 아이디가 test1-2인 버튼 클릭 시 > 아이디가 test1-1인 요소의 onclick 이벤트 리스너의 이벤트 핸들러 제거하기
document.querySelector("#test1-2").onclick = function() {
  // 기존 onclick의 이벤트 핸들러 함수를 null로 덮어씌움(이벤트 제거)
  test1a.onclick = null;
  alert("test1-1의 이벤트 제거");
}
...
// test1c 요소가 클릭되었을 때 배경색을 빨간색으로 변경
test1c.onclick = function() {
  test1c.style.backgroundColor = 'red';
}

// 배경이 빨간색에서 원상복구되고 글자색만 하얀색이 됨
// -> onclick에 저장된 값(이벤트 핸들러 함수)이 덮어씌워지면서 이전 코드가 사라지는 문제 발생
test1c.onclick = function() {
...
// 표준 이벤트 모델 작성법 : 요소.addEventListner("이벤트종류", 이벤트핸들러(함수));
test2.addEventListener("click", function() {
  ...
  if(test2.style.opacity == 0) {
    test2.style.opacity = 1; // 다시 불투명하게 바꾸기
  }
});

// #test-2 요소를 클릭하면 클릭한 횟수 카운트
let count = 0;
test2.addEventListener("click", function() {
  count++;

  // 표준 이벤트 모델의 이벤트 핸들러 내부에서 this == 이벤트가 발생한 요소
...
input3.addEventListener("keyup", function(e){
  if(e.key == "Enter") {
    // style 속성에 접근하면 > css 속성 대부분을 이용할 수 있다!
